...
userRouter.post ("/register", async (req: Request, res: Response) => {
    try {
        ...
        if (!username || !email || !password) {
            return res.status(StatusCodes.BAD_REQUEST).json({error : 'Please provide all the required parametes...'})
        }
...
userRouter.post ("/login", async (req: Request, res: Response) => {
    try {
        ...
        if (!email || !password) {
            return res.status(StatusCodes.BAD_REQUEST).json({error : "Please provide all the request parameters..."})
        }
